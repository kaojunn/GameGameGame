...
- 引擎会为每个选项生成展示模型（可用/不可用、提示文本、不可用原因）。
- **不可用选项不再隐藏**，而是以禁用态显示，避免玩家误以为“少了分支”。
- 不可用时当前实现会给出原因文案：
  - `RequiresItemId` 未满足：提示所需物品名（若目录有定义则展示名称）。
  - `RequiresInsightSum` 未满足：提示阈值与当前 `洞察 + 机巧` 数值。
- 若选项带检定（`Check`），会提示「`Stat + 1d10 ≥ Threshold`」以及当前对应属性值，方便读风险。

---

## 5. UI 与拖放（运行时生成）

右侧面板大致自上而下：**状态文本 → 装备槽（三格）→ 使用区（消耗品）→ 背包网格**。独立 **DragLayer** 用于拖动中置顶显示。底部选项区支持禁用态按钮与提示文字。
