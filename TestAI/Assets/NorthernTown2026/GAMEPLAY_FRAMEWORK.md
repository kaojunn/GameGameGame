...
**选项可见性与可选性**（全部显示；条件不足时禁用并显示门槛提示）：

- `RequiresItemId`：玩家背包需包含该 ID；若缺失，按钮置灰并显示「需要物品：xxx」。
- `RequiresInsightSum`：`洞察 + 机巧` 的有效值之和需 ≥ 该值；若不足，按钮置灰并显示「需要洞察+机巧 ≥ n（当前 m）」。
- 引擎层 `Choose` 会再次校验可选性，即使 UI 误触也不会执行结算，且会输出「条件不足」日志。
